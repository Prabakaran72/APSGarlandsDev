...
use Modules\Product\Entities\Preorder;
...
class Product extends Model
{
    ...
    // protected $preorderAttributes = ['prepare_days', 'pre_short_description', 'is_preorder_status'];
    ...
    public function scopeForCard($query)
    {
        $query
            ...
            ->withCount('options')
            ->with('reviews')
            ->addSelect(['products.id', 'products.slug', 'products.in_stock', 'products.manage_stock', 'products.qty', 'products.new_from', 'products.new_to', 'products.prepare_days']);
    }
    ...
    // {
    //     dd($this->id);
    //     // return $this->hasMany(static::class, 'preorder', 'product_id', 'prepare_days','short_description','is_preorder_status');
    // }
    public function preOrderProduct()
    {
        // dd($this->id);
...
// }
    ...
    public function saveRelations($attributes = [])
    {
        ...
        // $this->preOrderProduct()->sync(array_get($attributes, 'pre orders', []));
        // $preorderAttributes = [
        //     'product_id' => $this->id, // Use the current product's id
        //     'prepare_days' => array_get($attributes, 'prepare_days'),
